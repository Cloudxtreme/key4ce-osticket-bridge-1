...
if (count($result1) > 0) {
if($keyost_version==194 || $keyost_version==195 || $keyost_version==1951){
	 $ost_wpdb->insert($ticket_table, array('number' => $tic_ID, 'user_id' => $usid, 'user_email_id' => $usid,'status_id' => $ticketstate,'dept_id' => $dep_id, 'sla_id' => $sla_id, 'topic_id' => $top_id, 'staff_id' => $staff_id, 'team_id' => $team_id, 'email_id' => $usid, 'ip_address' => $ip_add, 'source' => $sour, 'isoverdue' => $isoverdue, 'isanswered' => $isans, 'lastmessage' => $las_msg, 'created' => $cre), array('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s'));
}else{
...
if (key4ce_getKeyValue('ticket_alert_dept_members') == 1 && key4ce_getKeyValue('ticket_alert_active') == 1) {
    ...
    if ($department_staff > 0) {
        /*array ( 0 => stdClass::__set_state(array( 'email' => 'leandromancino@gmail.com', 'firstname' => 'Leandro', 'lastname' => 'Mancino', )), )*/
        foreach ($staff_details as $staff) {
             #wp_mail("info@leandromancino.it","ehm",$staff->email); 

            $staff_firstname = $staff->firstname;
            $staff_lastname = $staff->lastname;
            $staff_email = $staff->email;
            $subject = "Nuovo Ticket";
            wp_mail($staff_email,$subject,"Nuovo ticket dalla piattaforma inserito nel tuo reparto."); 
